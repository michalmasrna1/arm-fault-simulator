...
if os.system("arm-none-eabi-gcc -Wall -Wpedantic -std=c11 -ffreestanding -nostdlib -mthumb -march=armv7-m -O0 -Wl,-Texample/linker.ld -Wl,-Map,example/tmp_data/binary.map example/example.c -o example/tmp_data/binary.elf") != 0: sys.exit(1)

# create disassembly for looking up armory results later
if os.system("arm-none-eabi-objdump example/tmp_data/binary.elf -d > example/tmp_data/disassembled.txt") != 0: sys.exit(1)
